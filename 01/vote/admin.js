...
app.get('/getMatchList', (req, res) => {
    let { userData, voteData, query } = req, { limit = 10, page = 1, search = '' } = query;
...
app.get('/vote', (req, res) => {
    let { participantId } = req.query, { userData, voteData } = req;
    if (req.session.userID) {
        //=>向投票表中存放内容
        voteData.push({
...
app.get('/getUser', (req, res) => {
    let { userData, query } = req, { userId = req.session.userID } = query,
    result = {
        code: 1,
        message: 'NO USER!',
        data: null
...
app.get('/checkUser', (req, res) => {
    let { userData, voteData, query } = req, { checkId } = query,
    result = {
        code: 1,
        message: 'NO!'
    };
...
app.get('/checkPhone', (req, res) => {
    let { userData, query } = req, { phone } = query,
...
app.post('/login', (req, res) => {
    let { userData, voteData, body } = req, { name = '', password = '' } = body;
